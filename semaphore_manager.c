...
//	panic("createSemaphore() is not implemented yet...!!");
	int check = get_semaphore_object_ID(ownerEnvID , semaphoreName) ;
	//cprintf(   "   check    %d    \n" , check);
	if(check != E_SEMAPHORE_NOT_EXISTS){
		return E_SEMAPHORE_EXISTS ;
	}else {
		struct Semaphore* allocatedObject;
		int r = allocate_semaphore_object(&allocatedObject) ;
		if(r != E_NO_SEMAPHORE){
			semaphores[r].ownerID = ownerEnvID;
			semaphores[r].value = initialValue ;
			strcpy(  semaphores[r].name ,semaphoreName );
			return r ;
		}
	}
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	int check = get_semaphore_object_ID(ownerEnvID , semaphoreName) ; //1
	if(check != E_SEMAPHORE_NOT_EXISTS){
		semaphores[check].value =semaphores[check].value -1 ;                                        //2
		if (semaphores[check].value < 0)                             //3
		{
			enqueue(&semaphores[check].env_queue, myenv);                                     //a
			myenv->env_status = ENV_BLOCKED;                          //b
			curenv  = NULL;                                           //c
		}
	}
	fos_scheduler();                                              //4
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	int check = get_semaphore_object_ID(ownerEnvID , semaphoreName) ; // 1
	if(check != E_SEMAPHORE_NOT_EXISTS){
		semaphores[check].value++;              //2                          // 2

		if (semaphores[check].value <= 0)                                // 3
		{
			env = dequeue(&semaphores[check].env_queue);                                       // a
			sched_insert_ready(env);                                  // b
			env->env_status = ENV_READY;                            // c
		}
	}
	// Steps:
	//	1) Get the Semaphore
