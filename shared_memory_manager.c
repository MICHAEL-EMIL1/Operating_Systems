...
int32 sharedID[PPN(USER_HEAP_MAX)] ;
int32 owneredID[PPN(USER_HEAP_MAX)] ;
uint32 sizeID[PPN(USER_HEAP_MAX)] ;
...
//	cprintf("(freePages - freePagesAfter)    %d\n" ,calculate_available_frames() ) ;
	if(Exist==E_SHARED_MEM_NOT_EXISTS){

		struct Share *newshare;
		uint32 rounded=ROUNDDOWN((uint32)virtual_address,PAGE_SIZE);
		uint32 endAddress=ROUNDUP((uint32)virtual_address+size,PAGE_SIZE);

		int IDshare=allocate_share_object(&newshare);
//		cprintf("E_SHARED_MEM_NOT_EXISTS    %d\n" ,calculate_available_frames() ) ;

		int framesCount=0;
		for(int i=rounded;i<endAddress;i+=PAGE_SIZE){
			framesCount++;
		}
		if(IDshare!=E_NO_SHARE){

			uint32 flag =allocate_chunk(myenv->env_page_directory, rounded, ROUNDUP(size,PAGE_SIZE), PERM_WRITEABLE|PERM_USER);
//			cprintf("noshare    %d\n" ,calculate_available_frames() ) ;

			if(flag!= -1){
				shares[IDshare].ownerID=ownerID;
				strcpy(shares[IDshare].name,shareName);
				shares[IDshare].size=ROUNDUP(size,PAGE_SIZE);
				shares[IDshare].references=1;
				shares[IDshare].isWritable=isWritable;
				for(int i=0;i<framesCount;i++){
					struct FrameInfo *shared_frame;
					allocate_frame(&shared_frame);
					shared_frame->va=rounded+(i*PAGE_SIZE);
					sharedID[PPN( rounded+(i*PAGE_SIZE))] = IDshare;
					owneredID[PPN( rounded+(i*PAGE_SIZE))] = ownerID;
					sizeID[PPN( rounded+(i*PAGE_SIZE))] = endAddress;
					shared_frame->environment=myenv;
					map_frame(myenv->env_page_directory, shared_frame, rounded+(i*PAGE_SIZE), PERM_WRITEABLE|PERM_USER);
					add_frame_to_storage(shares[IDshare].framesStorage,shared_frame,i);
				}
//				cprintf("(fre%d\n"   , framesCount);
...
//	cprintf("gettst()     %d\n" , 2) ;
	...
	uint32 id=get_share_object_ID(ownerID,shareName);
	int j =0 ;
	int ret;
	uint32 va=ROUNDDOWN((uint32)virtual_address,PAGE_SIZE);
	uint8 perm  ;
	if( shares[id].isWritable  ){
		perm = PERM_WRITEABLE|PERM_USER ;
	}else{
		perm = PERM_USER ;
	}
	...
	if(id!=E_SHARED_MEM_NOT_EXISTS){
		for(uint32 i= 0 ; i>=0 ;i++){
			if(shares[id].framesStorage != NULL){
			struct FrameInfo * f =get_frame_from_storage(shares[id].framesStorage,i);
			if( f!=NULL){
				map_frame(myenv->env_page_directory,f,va , perm);
			}else{
				break;
			}
			va+=PAGE_SIZE;
...
int freeSharedObject(int32 sharedObjectID, void *startVA)
{
	...
	//	a) 0 if success
	//	b) E_SHARED_MEM_NOT_EXISTS if the shared object is not exists
	sharedObjectID = sharedID[PPN(startVA)];
	uint32 ownerID =owneredID[PPN(startVA)];
//	cprintf("(freeFrames - sys_calculate_free_frames())   %d"   , 1);

	int id = get_share_object_ID(ownerID ,shares[sharedObjectID].name);
//	cprintf("(freeFrames - sys_calculate_free_frames())   %d"   , 9);

	if(id == E_SHARED_MEM_NOT_EXISTS){
//		cprintf("(freeFrames - sys_calculate_free_frames())   %d"   , 4);
		return E_SHARED_MEM_NOT_EXISTS;
	}else{
		uint32 index = 1;
		uint32 *ptr_page_table = NULL ;
		struct FrameInfo *frame_info=NULL;
		for(uint32 i = ROUNDDOWN((uint32)startVA, PAGE_SIZE) ; i < sizeID[PPN(startVA)] ; i+=PAGE_SIZE ){
//			cprintf("(freeFrames - sys_calculate_free_frames())   %d\n"   , 2);
			int emptyTable=0;
			frame_info= get_frame_info(curenv->env_page_directory,i,&ptr_page_table) ;
			get_page_table(curenv->env_page_directory,i,&ptr_page_table) ;

			if(frame_info != NULL){
				unmap_frame(curenv->env_page_directory ,i);
				for(int x=0;x<1024;x++){
					...
					if(emptyTable==1024){
						...
						curenv->env_page_directory[PDX(i)] = 0;
					}
				}
//				cprintf("shares[sharedObjectID].references  %d\n"   , shares[sharedObjectID].references);

			}
		}
		//env_page_ws_clear_entry(myenv, ownerID ) ;
